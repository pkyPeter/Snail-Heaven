...
class Email extends React.Component {
        ...
        <div className="email">
          <div className="background">
            <div className="form">
              ...
              <div className="title">
                寄信給 {this.props.toggleEmail.currentDetail.host_name}
              </div>
              <form id="checkAvailability">
                <input
                  className="name formChild"
                  type="text"
                  placeholder="請輸入姓名"
                  required
                />
                <input
                  className="senderEmail formChild"
                  type="email"
                  placeholder="Email"
                  required
                />
                <input
                  className="phone formChild"
                  type="tel"
                  placeholder="電話"
                />
                <input
                  className="date formChild"
                  type="date"
                  placeholder="預計起租日期："
                />
                <input
                  className="visitTime formChild"
                  placeholder="希望拜訪時間"
                />
                <textarea
                  className="message formChild"
  ...
  sendEmail(e) {
    ...
    let info = {
      loadgerName: loadgerName,
      lodgerEmail: lodgerEmail,
      lodgerPhone: lodgerPhone,
    ...
    if (
      ...
      fetch(
        "https://us-central1-snail-heaven-1537271625768.cloudfunctions.net/app/exe/sendemail",
        {
      ...
      lib.func.get(
        ".remind"
      ).innerHTML = `以下欄位均為必填：<br />${message}，謝謝您。`;
