...
class PriceChart extends React.Component {
	...
	componentDidMount(){
		...
		if ( (this.props.filters.priceCeiling !== this.state.priceCeiling || this.props.filters.priceFloor !== this.state.priceFloor) && this.state.priceCeiling === 100000) {
			console.log("c9l;dkjf;lskdjg;lsdkjf")
		...
		return (
			<div className="filterDetail priceChart">
				...
				<div className="control">
				    <div className="circle left" style={{left: this.state.left}} onMouseDown={this.startToMove} onTouchStart={this.startToMove}>
	...
	currentLeft(event) {
		// 首先把位置以及價錢做出
		    let width = lib.func.get(".control").offsetWidth;
	   		let screenX = event.type === "mouseup" ? event.screenX : event.changedTouches[0].screenX;
	...
	confirmLeft(event) {
	    let width = lib.func.get(".control").offsetWidth;
	   	let screenX = event.type === "mousemove" ? event.screenX : event.changedTouches[0].screenX;
	    let finalLeft = this.state.left + (screenX - this.state.originXLeft);
	    //這邊的16是width加上border
	    if (finalLeft < 0 ) { finalLeft = 0} else if (finalLeft > width-16) { finalLeft= (width-16) };
	    //顯示區間數字，將區間數字轉換為５００為單位
	...
	currentRight(event) {
	    ...
	    if ( event.type === "mouseup") {
	   	...
	   	if ( this.state.leftMoney > moveCashTranform ) {
	   		...
	   		console.log("moveCashTranform at props:" + moveCashTranform)
	   		this.props.changeFilters("priceCeiling", moveCashTranform);
	   	}
		this.defineChartRange();
		//距離太近時，價錢進行合併
